...
- ✍️ **生成控制**：续写（`/酒续写`）、备选回复切换（`/酒备选`）、中断生成（`/酒中断`）、新对话（`/酒新建`）
- 🛠️ **角色卡工具**：文字建卡（`/酒加卡 名字 [角色描述] [开场白]`，无需图片；同名卡存在时改为更新对应字段，支持 `[/]`=不改 与 `[]`=清空）
- 📊 **数据工具**：聊天记录导出（`/酒导出`，当前聊天存为 JSONL 发送）、聊天统计（`/酒统计`）
...
| `/酒加卡 <图片>` 或 `/酒加卡 名字 [角色描述] [开场白]` | `tavern_add` | PNG 图片，或 名字 + 中括号包裹的描述/开场白 | 上传角色卡到酒馆：附带图片（直接附带、引用含图消息、先发指令后补发）；或纯文字建卡，描述与开场白用中括号包裹防止空格换行出错，开场白可省略；同名卡已存在时改为更新：`[/]`=保留原值、`[]`=清空、省略括号=不改 |
...
| `/酒人设 修改 名字 [内容]` | `tavern_persona modify` | 名字 + 中括号包裹的描述内容（可选） | 新建或修改人设：内容用中括号包裹可含空格换行；人设不存在则新建（仅名字为空人设），存在且有内容则改描述，存在且无内容则删除 |
...
|    文字建卡    | `/酒加卡 猫娘 [你是一只猫娘] [喵~]`     | 无需图片直接创建角色卡，描述与开场白用中括号包裹 |
|    更新已有卡    | `/酒加卡 猫娘 [/] [喵~]`     | 同名卡存在时改为更新：`[/]`=不改，`[]`=清空，省略括号=不改 |
...
> ⚠️ **注意**：`low_memory_mode` 开启后，每次命令执行前后都会自动启动和关闭浏览器，内存占用低但响应较慢；关闭后浏览器常驻后台，响应更快。两种模式均已防止浏览器进程残留；异常时可用 `/酒进程` 查看进程数量与状态，用 `/酒进程 停止` 仅清理浏览器，用 `/酒停止` 完全关闭（含酒馆主程序）。
