...
- 🖥️ **WebUI 控制面板**：AstrBot 面板内卡片式控制面板，支持状态检测、一键启动/关闭/重启/安装/卸载/导入导出 SillyTavern（需 AstrBot ≥ v4.24.2）
...
- 📦 **一键部署**：内置 SillyTavern 下载脚本，WebUI 支持一键安装/卸载酒馆及导入导出数据
...
在 AstrBot WebUI 中进入 **插件 → astrbot_plugin_bartender_extra → 详情页 → Pages「酒馆界面」**，即可看到控制面板。

面板功能：

- **状态栏**：显示酒馆地址与在线状态（绿/灰圆点），右侧「复制」「刷新」按钮
- **酒馆控制**：根据酒馆运行状态自动切换按钮
  - 未启动时显示「启动」按钮
  - 运行中显示「关闭」「重启」两个按钮
- **外部打开**：在新标签页中打开 SillyTavern 界面
- **安装/卸载**：一键安装最新版 SillyTavern，或删除整份数据
- **数据管理**：导入 zip 备份恢复数据，或导出整库备份为 zip 下载
- 所有按钮根据酒馆状态自动启用/禁用，操作结果实时反馈

> 酒馆地址取自插件配置中的 `browser_ip` 与 `browser_port`，修改配置后刷新页面即生效。

> ⚠️ **已知限制**：从**其他设备**访问面板时，配置中的 `127.0.0.1` 会指向那台设备自身，请将 `browser_ip` 改为服务器局域网 IP。
