...
| `/酒重生成` | 无 | 重新生成当前最新楼层的 AI 回复 |
| `/酒续写` | 无 | 续写最新楼层，让 AI 在最新回复后继续生成内容 |
| `/酒备选 [上\|下]` | 上/下（可选，默认下） | 切换最新楼层的上一条/下一条备选回复；下一条会重新生成 |
| `/酒停止生成` | 无 | 中断当前正在进行的酒馆生成 |
| `/酒新建` | 无 | 与当前角色开始新对话（清空当前楼层） |
| `/酒导出` | 无 | 导出当前聊天记录为 JSONL 文件发送 |
| `/酒统计` | 无 | 查看当前角色的聊天统计（消息数/字数/生成耗时等） |
| `/酒改名 [旧名] [新名]` | 旧名 + 新名 | 重命名指定角色卡，禁止修改默认角色名 |
| `/酒查看` | 无 | 查看最新楼层；引用一条消息后发送可定位其楼层数 |
| `/酒人设` | 无 | 查看当前绑定人设与酒馆人设列表 |
| `/酒人设 绑定 [名字]` | 人设名称 | 绑定当前用户到指定人设，之后 `/酒` 与 `/酒重生成` 会先切换人设 |
| `/酒人设 查看 [名字]` | 人设名称（可选） | 查看指定人设的名称、描述与位置等详细信息；无参时展示人设列表 |
| `/酒人设 修改 [名字] [内容]` | 名字 + 描述内容（可选） | 新建或修改人设：人设不存在则新建（仅名字为空人设），存在且有内容则改描述，存在且无内容则删除 |
| `/酒人设 解绑` | 无 | 解除当前用户绑定的人设 |
| `/酒世界书` | 无 | 列出所有世界书与当前已启用的世界书 |
| `/酒世界书 查看 [名字]` | 世界书名称 | 查看指定世界书的条目内容（触发词与正文） |
| `/酒世界书 切换 [名字]` | 世界书名称 | 启用或停用指定世界书（对当前角色生效） |
| `/酒开始 群聊` | 群聊（可选字面量） | 开启酒馆聊天模式：之后直接发消息即转酒馆，无需 `/酒` 前缀；带「群聊」按当前群生效，无参按本人；同一用户只能开启一次 |
| `/酒结束` | 无 | 结束自己创建的酒馆聊天模式 |
| `/酒状态` | 无 | 查看当前角色卡、角色列表及浏览器连接状态 |
| `/酒帮助` | 无 | 显示所有指令的帮助信息 |
| `/酒启动` | 🔒 管理员 | 启动插件目录中的 SillyTavern 酒馆服务，启动后自动连接浏览器 |
| `/酒关闭` | 🔒 管理员 | 关闭插件浏览器并清理后台所有 Chrome/Chromium 进程 |
| `/酒重置` | 🔒 管理员 | 重置插件所有全局变量，恢复默认角色，重新获取角色列表 |
| `/酒进程` | 🔒 管理员 | 查看当前 Chrome/Chromium 进程数量与插件浏览器状态 |
| `/酒权限` | 🔒 管理员 | 管理酒命令访问权限：`管理 开\|关`（管理员模式）、`白名单 [群号\|移除 群号]`、`黑名单 [群号\|移除 群号]` |
