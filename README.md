...
- 🛠️ **角色卡工具**：文字建卡（`/酒加卡 名字 [角色描述] [开场白]`，无需图片）、聊天记录导出（`/酒导出`）、聊天统计（`/酒统计`）
...
| `/酒加卡 [图片]` 或 `/酒加卡 名字 [角色描述] [开场白]` | `tavern_add` | PNG 图片，或 名字 + 中括号包裹的描述/开场白 | 上传角色卡到酒馆：附带图片（直接附带、引用含图消息、先发指令后补发）；或纯文字建卡，描述与开场白用中括号包裹防止空格换行出错，开场白可省略 |
