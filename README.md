...
| `/酒继续` | 无 | 续写最新楼层，让 AI 在最新回复后继续生成内容 |
| `/酒swipe [上\|下]` | 上/下（可选，默认下） | 切换最新楼层的上一条/下一条备选回复；下一条会重新生成 |
| `/酒停止生成` | 无 | 中断当前正在进行的酒馆生成 |
| `/酒新聊` | 无 | 与当前角色开始新对话（清空当前楼层） |
| `/酒导出` | 无 | 导出当前聊天记录为 JSONL 文件发送 |
| `/酒统计` | 无 | 查看当前角色的聊天统计（消息数/字数/生成耗时等） |
| `/酒改名 [旧名] [新名]` | 旧名 + 新名 | 重命名指定角色卡，禁止修改默认角色名 |
| `/酒查看` | 无 | 查看最新楼层；引用一条消息后发送可定位其楼层数 |
| `/酒人设 [名字]` | 人设名称（可选） | 绑定当前用户到指定人设，之后 `/酒` 与 `/酒重新` 会先切换人设；无参数时查看绑定状态与人设列表 |
| `/酒人设修改 [名字] [内容]` | 名字 + 描述内容（可选） | 新建或修改人设：人设不存在则新建（仅名字为空人设），存在且有内容则改描述，存在且无内容则删除 |
| `/酒人设解绑` | 无 | 解除当前用户绑定的人设 |
...
| `global_persona_binding` | bool | `false` | 全局人设绑定：开启后同一用户在所有群共用一个人设（按QQ号）；关闭时按群隔离，不同群可绑定不同人设 |
