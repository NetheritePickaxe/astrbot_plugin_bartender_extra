...
### 变更
- WebUI 控制面板完全重写：卡片式布局，3 列网格，分类标题（服务控制/安装卸载/数据管理）
- 按钮根据酒馆状态自动显隐（未启动→仅「启动」；运行中→「关闭」「重启」）
- 刷新按钮移至状态栏（复制按钮右侧），不再占用独立卡片
- 启动/关闭/重启合并为「酒馆控制」卡片，状态感知切换显示
- 所有按钮改为禁用灰色样式而非隐藏，状态更清晰
- 面板布局居中放大，最大宽度 1100px，适配深色/浅色主题
- 修复 JS `export-data` 对象键未加引号导致整页脚本不执行的严重问题
- 修复 `applyStatus` 空值保护与 catch 分支缺失按钮状态兜底
- 重构 `app.js`：`setStMode` 管理按钮可见性，`setAllButtons` 管理禁用状态，逻辑分离避免冲突
- 重写 `style.css`：卡片/下拉/分割按钮/状态点样式，暗色主题适配
...
### 修复
- 修复控制面板按钮不显示的问题：v1.7.0 的 `revealAction`/`hideActions` 系统每次只显示一个按钮，导致多个按钮无法同时出现
- 修复顶栏布局：改为居中单卡片面板，状态行 + 2 列按钮网格，更简洁直观

### 变更
- 重写 `app.js`：移除 `revealAction`/`hideActions`/`KNOWN_ACTIONS`/`localStorage` 系统，改用 `showButtons(ids)` 按状态分组显隐按钮
- 重写 `index.html`：移除顶栏布局，改为居中面板，包含状态行 + 按钮网格 + 消息反馈区
- 重写 `style.css`：移除顶栏/卡片/分体/iframe 样式，添加居中面板/按钮网格/消息样式
...
### 变更
- 移除内嵌酒馆 iframe，改为卡片式控制面板：常用操作一键触达，不再受 Dashboard 沙箱/iframe 限制问题困扰
- 面板提供「启动酒馆」「关闭酒馆」「重启酒馆」「安装酒馆」「在新标签页打开酒馆」「导出整库备份」等按钮，操作结果实时反馈
- 新增 `GET {PLUGIN_NAME}/tavern/export-data` Web API，调用 `file_response` 直接返回 zip 文件供浏览器下载
- 因不再需要内嵌，移除 `_patch_st_helmet` 调用（ST 的 helmet 安全头不受影响）
